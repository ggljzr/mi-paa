...
{
    DATA new_data;
    new_data.m_id = data->m_id;
    new_data.m_n = data->m_n;
    new_data.m_max_weight = data->m_max_weight;

    new_data.m_items = (ITEM *) calloc (data->m_n, sizeof(ITEM));

    int i = 0;
    for(i = 0; i < data->m_n; i++)
    {
        new_data.m_items[i].m_item_id = data->m_items[i].m_item_id;
        new_data.m_items[i].m_value = data->m_items[i].m_value;
        new_data.m_items[i].m_weight = data->m_items[i].m_weight;
        new_data.m_items[i].m_ratio = data->m_items[i].m_ratio;
    }
...
RESULT knapsack_bruteforce(DATA * data)
{
    ...
    for(i = 0; i < pow_n; i++)
    {
        ...
        {
            if((value_sum > result.m_value) ||
                    (value_sum == result.m_value && weight_sum < result.m_weight))
            {
